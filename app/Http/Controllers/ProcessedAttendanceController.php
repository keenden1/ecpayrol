    ...
    try {
        ...
        foreach ($attendances as $attendance) {
            ...
            if ($attendance->time_in) {
                // Calculate late minutes - Using employee schedule for accurate expected times
                try {
                    $attendanceDate = \Carbon\Carbon::parse($attendance->attendance_date);
                    $actualTimeIn = \Carbon\Carbon::parse($attendance->time_in);

                    // Get employee's schedule to determine expected time in
                    $dayOfWeek = strtolower($attendanceDate->format('l')); // monday, tuesday, etc.
                    $schedule = \App\Models\EmployeeSchedule::where('employee_id', $attendance->employee_id)
                    ...
                    // Determine time_out for shift detection
                    ...
                    if ($schedule && $schedule->start_time) {
                        $scheduledStart = \Carbon\Carbon::parse($schedule->start_time);
                        $expectedTimeIn = $attendanceDate->copy()->setTime($scheduledStart->hour, $scheduledStart->minute, 0);
                        Log::info("Using employee schedule for expected time", [
                            'attendance_id' => $attendance->id,
                            'expected_time_in' => $expectedTimeIn->format('H:i:s'),
                            'shift_type' => $schedule->shift_type ?? 'unknown'
                        ]);
                    } else {
                        // Fallback to time-based detection if no schedule found
                        Log::info("No schedule found - using time-based detection", [
                            'attendance_id' => $attendance->id,
                            'employee_id' => $attendance->employee_id,
                            'work_day' => $dayOfWeek
                        ]);
                        ...
                        // Night shift: time_out around 6-8 AM next day
                            ...
                            $expectedTimeIn = $attendanceDate->copy()->setTime(17, 0, 0); // 5:00 PM
                            ...
                            if ($timeInHour >= 6 && $timeInHour <= 10) {
                                ...
                                $expectedTimeIn = $attendanceDate->copy()->setTime(17, 0, 0); // 5:00 PM (Night Shift)
                            } else {
                ...
                if ($isHalfday) {
                    ...
                    // FIXED: Calculate undertime based on 8-hour standard work requirement
                    $standardWorkHours = 8;
                    $standardWorkMinutes = $standardWorkHours * 60; // 8 hours = 480 minutes
                    $netWorkedMinutes = $hoursWorked * 60;

                    if ($netWorkedMinutes < $standardWorkMinutes) {
                        $undertimeMinutes = round($standardWorkMinutes - $netWorkedMinutes, 2);
                    } else {
                        $undertimeMinutes = 0;
                    }
                    ...
                    Log::info("Undertime calculation", [
                        ...
                        'is_undertime' => $hoursWorked < $standardWorkHours
                    ]);
                }
                    ...
                    if ($attendance->time_in) {
                        ...
                        try {
                            ...
                            if ($timeInHour >= 6 && $timeInHour <= 10) {
                                ...
                                // Night shift: 5:00 PM
                                $expectedTimeIn = $attendanceDate->copy()->setTime(17, 0, 0);
                            } else {
...
/**
 * Bulk update attendance records from inline table editing
 */
public function bulkUpdate(Request $request)
{
    try {
        $validator = Validator::make($request->all(), [
            ...
            'records.*.id' => 'required|integer|exists:processed_attendances,id',
        ]);
        ...
        foreach ($records as $recordData) {
            try {
                $attendance = ProcessedAttendance::find($recordData['id']);

                if (!$attendance) {
                    $errors[] = "Record ID {$recordData['id']} not found";
                    continue;
                }
                ...
                $updateData = [];
                $allowedFields = [
                    'time_in', 'time_out', 'break_in', 'break_out', 'next_day_timeout',
                    'hours_worked', 'overtime', 'travel_order', 'slvl', 'holiday',
                    'offset', 'trip', 'ot_reg_holiday', 'ot_special_holiday',
                    'late_minutes', 'undertime_minutes', 'ct', 'cs', 'restday'
                ];
                ...
                foreach ($allowedFields as $field) {
                    if (array_key_exists($field, $recordData)) {
                        ...
                        // Handle time fields - convert to proper datetime format
                        if (in_array($field, ['time_in', 'time_out', 'break_in', 'break_out', 'next_day_timeout'])) {
                            if (!empty($value)) {
                                // If it's just a time (HH:MM), combine with attendance date
                                if (preg_match('/^\d{2}:\d{2}$/', $value)) {
                                    $dateToUse = $attendance->attendance_date;

                                    // For next_day_timeout, use next day
                                    if ($field === 'next_day_timeout') {
                                        $dateToUse = Carbon::parse($attendance->attendance_date)->addDay()->format('Y-m-d');
                                    }

                                    $value = $dateToUse . ' ' . $value . ':00';
                                }
                        ...
                        elseif (in_array($field, ['hours_worked', 'overtime', 'travel_order', 'slvl', 'holiday', 'offset', 'trip', 'ot_reg_holiday', 'ot_special_holiday', 'late_minutes', 'undertime_minutes'])) {
                            $updateData[$field] = is_numeric($value) ? floatval($value) : 0;
                        }
                        // Handle boolean fields
                        elseif (in_array($field, ['ct', 'cs', 'restday'])) {
                            $updateData[$field] = filter_var($value, FILTER_VALIDATE_BOOLEAN);
                        }
                        else {
                            $updateData[$field] = $value;
                        }
                ...
                Log::info("Bulk update: Updated attendance record", [
                    'id' => $attendance->id,
                    'employee_id' => $attendance->employee_id,
                ...
                $errors[] = "Error updating record ID {$recordData['id']}: " . $e->getMessage();
                Log::error("Bulk update error for record {$recordData['id']}: " . $e->getMessage());
            }
        }
