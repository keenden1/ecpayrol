                            ...
                            <TabsContent value="attendance" className="space-y-4">
                                {employeeDetails.attendance_summary ? (
                                    <>
                                            ...
                                            <CardContent className="p-6">
                                                <div className="grid grid-cols-4 gap-4">
                                                    <div>
                                                        ...
                                                        <p className="text-2xl font-bold">{employeeDetails.attendance_summary.summary?.total_records || 0}</p>
                                                    </div>
                                                    <div>
                                                        <span className="text-sm text-gray-500">Days Present:</span>
                                                        <p className="text-2xl font-bold text-green-600">{employeeDetails.attendance_summary.summary?.days_present || 0}</p>
                                                    </div>
                                                    <div>
                                                        <span className="text-sm text-gray-500">Days Absent:</span>
                                                        <p className="text-2xl font-bold text-red-600">{employeeDetails.attendance_summary.summary?.days_absent || 0}</p>
                                                    </div>
                                                    <div>
                                                        <span className="text-sm text-gray-500">Hours Worked:</span>
                                                        <p className="text-2xl font-bold">{employeeDetails.attendance_summary.summary?.total_hours_worked || 0}</p>
                                                    </div>
                                                </div>
                                            </CardContent>
                                        </Card>
                                        ...
                                        {/* DTR Table */}
                                        {employeeDetails.attendance_summary.records?.length > 0 ? (
                                            <Card>
                                                <CardHeader>
                                                    <CardTitle className="text-lg">Daily Time Record (DTR)</CardTitle>
                                                </CardHeader>
                                                        ...
                                                        <table className="min-w-full divide-y divide-gray-200">
                                                            <thead className="bg-gray-50">
                                                                <tr>
                                                                    ...
                                                                    <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Time In</th>
                                                                    <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Break Out</th>
                                                                    <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Break In</th>
                                                                    <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Time Out</th>
                                                                    <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Late/Under</th>
                                                                    <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Hours</th>
                                                                    <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">OT</th>
                                                                    <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">SLVL</th>
                                                                    <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Flags</th>
                                                                </tr>
                                                            </thead>
                                                            <tbody className="bg-white divide-y divide-gray-200">
                                                                {employeeDetails.attendance_summary.records.map((record, index) => {
                                                                    const lateUnder = (record.late_minutes || 0) + (record.undertime_minutes || 0);
                                                                    const flags = [];
                                                                    if (record.ct) flags.push('CT');
                                                                    if (record.cs) flags.push('CS');
                                                                    if (record.ob) flags.push('OB');
                                                                    if (record.restday) flags.push('RD');
                                                                    if (record.holiday > 0) flags.push('HOL');
                                                                    if (record.is_nightshift) flags.push('NS');
                                                                    ...
                                                                    return (
                                                                        <tr key={index} className="hover:bg-gray-50">
                                                                            ...
                                                                            <td className="px-3 py-2 whitespace-nowrap text-sm">
                                                                                {formatTime(record.time_out || record.next_day_timeout)}
                                                                            </td>
                                                                            <td className="px-3 py-2 whitespace-nowrap text-sm">
                                                                                {lateUnder > 0 ? (
                                                                                    <span className="text-red-600">{lateUnder} min</span>
                                                                                ) : '-'}
                                                                            </td>
                                                                            ...
                                                                            <td className="px-3 py-2 whitespace-nowrap text-xs">
                                                                                {flags.length > 0 ? (
                                                                                    <div className="flex flex-wrap gap-1">
                                                                                        {flags.map((flag, i) => (
                                                                                            <span key={i} className="inline-flex px-2 py-0.5 rounded-full text-xs font-semibold bg-blue-100 text-blue-800">
                                                                                                {flag}
                                                                                            </span>
                                                                                        ))}
                                                                                    </div>
                                            ...
                                            <p className="text-gray-500 italic text-center py-8">No detailed attendance records found for this period</p>
                                        )}
                                    </>
                                ) : (
